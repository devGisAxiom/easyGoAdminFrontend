...
class CancelledRides extends StatefulWidget {
  @override
  State<CancelledRides> createState() => _CancelledRidesState();
}
...
class _CancelledRidesState extends State<CancelledRides> {
  ...
  Future<void> fetchdata() async {
    ...
    try {
      ...
      _allBikes = List.from(_bookingController.cancelledList);
      _filteredBikes = List.from(_allBikes);
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error loading cancelled rides: $e')),
      );
    } finally {
      ...
      if (query.isEmpty) {
        ...
        _filteredBikes =
            _allBikes.where((booking) {
              return (booking.bId?.toString() ?? '').toLowerCase().contains(
                    query,
                  ) ||
                  (booking.uName ?? '').toLowerCase().contains(query) ||
                  (booking.bUId?.toString() ?? '').toLowerCase().contains(
                    query,
                  ) ||
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailRow(
                    'Pickup Date',
                    _formatDate(
                      _parseDate(booking.bPickupDate.toString().toString()),
                    ),
                  ),
                  _buildDetailRow(
                    'Pickup Time',
                    booking.bPicupTime.toString().toString() ?? 'N/A',
                  ),
                  _buildDetailRow(
                    'Drop Date',
                    _formatDate(
                      _parseDate(booking.bDropDate.toString().toString()),
                    ),
                  ),
                  _buildDetailRow(
                    'Drop Time',
                  ...
                  _buildDetailRow(
                    'Cancellation Reason',
                    booking.viewReason ?? 'Not specified',
                  ),
                  _buildDetailRow(
                    'Booking Date',
                    _formatDate(_parseDate(booking.bookingDate.toString())),
                  ),
                  SizedBox(height: 16),
                  Container(
          ...
          (context) => AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Container(
                ...
                child: Text(
                  content.isNotEmpty ? content : 'No viewReason specified',
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Search Cancelled Rides',
                        hintText: 'Search by ID, user, viewReason, location...',
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          '${_filteredBikes.length} cancelled ride(s)',
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w500,
                            color: Colors.grey[600],
